...
export default function Dashboard() {
  ...
  const subjectsByDepartment = useMemo(() => {
    const counts = subjects.reduce<Record<string, number>>((acc, subject) => {
      const departmentName = (subject as { department?: { name?: string}}).department?.name ?? 'unknown'; 
      acc[departmentName] = (acc[departmentName] || 0) + 1;
      return acc;
    }, {})

    return Object.entries(counts).map(([departmentName, total]) => ({ departmentName, total}))
  }, [subjects])

  const classesBySubject = useMemo(() => {
    const counts = classes.reduce<Record<string, number>>((acc, classItem) => {
      const subjectName = classItem.subject?.name ?? 'unknown';
      acc[subjectName] = (acc[subjectName] || 0) + 1;
      return acc;
    }, {})

    return Object.entries(counts).map(([subjectName, total]) => ({ subjectName, total}))
  }, [classes])
